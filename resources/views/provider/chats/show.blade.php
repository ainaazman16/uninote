...
@section('content')
    <div class="container mt-4">
        ...
        <div class="d-flex align-items-center justify-content-between mb-3">
            <a href="{{ route('provider.chats.list') }}" class="btn btn-outline-secondary btn-sm">
                ← Back to All Chats
            </a>

            <h5 class="fw-bold mb-0">
                Chat with {{ $chat->student->name ?? 'Student' }}
            </h5>
        </div>
        ...
        <div class="card shadow-sm border-0">
            ...
            {{-- Send message --}}
            <form method="POST" action="{{ route('provider.chats.send', $chat) }}" class="card-footer bg-white"
                id="chat-form">
                @csrf
    ...
    <script>
        ...
        function fetchNewMessages() {
            fetch(`/provider/chats/${chatId}/messages`)
