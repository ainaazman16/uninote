    ...
    <div class="container mt-4">
        ...
        <form method="GET" class="mb-4">
                ...
                <div class="col-md-6">
                    <div class="input-group">
                        ...
                        <input type="text" name="search" class="form-control"
                            placeholder="Search by name, email, or motivation..." value="{{ request('search') }}">
                    </div>
                </div>
                <div class="col-md-4">
            ...
            @if (request('search') || request('status'))
                <div class="mt-2">
                    <a href="{{ route('admin.provider.applications') }}" class="btn btn-sm btn-outline-secondary">
                    ...
                    @forelse($applications as $app)
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                {{-- View Profile --}}
                                <a href="{{ route('admin.users.show', $app->user->id) }}" class="btn btn-info btn-sm mb-1">
                                    View Profile
                                </a>
                                ...
                                @if ($app->status == 'pending')
                                    <form action="{{ route('admin.provider.approve', $app->id) }}" method="POST">
                                        @csrf
                                        <textarea name="admin_comment" class="form-control mb-2" placeholder="Optional feedback"></textarea>
                                        <button class="btn btn-success btn-sm w-100">Approve</button>
                                    </form>

                                    <form action="{{ route('admin.provider.reject', $app->id) }}" method="POST"
                                        class="mt-2">
                                        @csrf
                                        <textarea name="admin_comment" class="form-control mb-2" placeholder="Reason for rejection" required></textarea>
