            ...
            <div class="card shadow-lg border-0 overflow-hidden rounded-4">
                <div class="row g-0">
                    ...
                    <div class="col-md-4 bg-light border-end d-flex flex-column align-items-center text-center p-5">
                        ...
                        <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" id="profileForm" class="w-100">
                        @csrf
                        @method('PATCH')

                        {{-- Image Wrapper --}}
                        <div class="position-relative mb-4">
                            <div class="ratio ratio-1x1 rounded-circle overflow-hidden shadow-sm border border-3 border-white" style="width: 150px; margin: 0 auto;">
                            ...
                            <label for="photoInput" class="btn btn-primary btn-sm rounded-circle position-absolute bottom-0 end-0 translate-middle-x mb-1 shadow-sm" style="width: 35px; height: 35px; display: flex; align-items: center; justify-content: center; cursor: pointer;">
                            ...
                            {{-- Hidden Input --}}
                            <input type="file" name="profile_photo" id="photoInput" class="d-none" accept="image/*" onchange="previewImage(event)">
                        </div>
                    ...
                    <div class="col-md-8 p-4 p-md-5">
                        ...
                        <h5 class="mb-4 text-uppercase fw-bold text-secondary small tracking-wide">Public Profile</h5>

                        {{-- Name (Read Only) --}}
                        <div class="form-floating mb-4">
                            <input type="text" class="form-control bg-light" id="name" value="{{ auth()->user()->name }}" disabled readonly>
                            <label for="name">Full Name <small class="text-muted">(Contact admin to change)</small></label>
                        </div>
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-12">
                                <div class="form-floating">
                                    <input type="text" name="university" class="form-control @error('university') is-invalid @enderror" 
                                           id="uni" value="{{ old('university', auth()->user()->university) }}" placeholder="University">
                            ...
                            <div class="col-md-8">
                                <div class="form-floating">
                                    <input type="text" name="programme" class="form-control @error('programme') is-invalid @enderror" 
                                           id="prog" value="{{ old('programme', auth()->user()->programme) }}" placeholder="Programme">
                            ...
                            <div class="col-md-4">
                                <div class="form-floating">
                                    <select name="year_of_study" class="form-select @error('year_of_study') is-invalid @enderror" id="year">
                                        <option value="" disabled>Select Year</option>
                                        @foreach(['1st Year', '2nd Year', '3rd Year', '4th Year', 'Postgrad'] as $year)
                                            <option value="{{ $year }}" {{ auth()->user()->year_of_study == $year ? 'selected' : '' }}>{{ $year }}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="form-floating mb-4">
                            <textarea name="bio" class="form-control @error('bio') is-invalid @enderror" 
                                      id="bio" style="height: 120px" placeholder="Bio">{{ old('bio', auth()->user()->bio) }}</textarea>
